...
import {
  BuildSummaryStepDurationSpan,
  BuildSummaryBuildDurationSpan,
} from './BuildSummaryDurationSpan';
...
const BuildSummaryStepLine = observer(
  ({build, step, parentFullDisplay}: BuildSummaryStepLineProps) => {
    ...
    const [fullDisplay, setFullDisplay] = useStateWithParentTrackingWithDefaultIfNotSet(
      parentFullDisplay,
      () => !step.complete || step.results !== SUCCESS,
    );
    ...
    const maybeRenderArrowExpander = () => {
      if (
        logs.array.length > 0 ||
        stepInfo.buildrequests.length > 0 ||
      ...
      return getStepLogsInDisplayOrder(logs).map((log) => {
        ...
        return (
          <LogPreview
            ...
            log={log}
            initialFullDisplay={initialFullDisplay}
          />
        );
      });
    };
...
export const BuildSummary = observer(
  ({build, parentBuild, parentRelationship, condensed}: BuildSummaryProps) => {
    ...
    const [detailLevel, setDetailLevel] = useState<DetailLevel>(
      condensed ? DetailLevel.None : DetailLevel.Everything,
    );
