...
export const BuildLinkWithSummaryTooltip = observer(
  ({build, builder}: BuildLinkWithSummaryTooltipProps) => {
    if (builder === null) {
      builder = undefined;
    }
    ...
    const renderBuildTooltip = (props: {[p: string]: any}) => (
      <Tooltip
        className={'buildsummarytooltipstyle ' + results2class(build, null)}
        id="bb-tooltip-build"
        {...props}
      >
