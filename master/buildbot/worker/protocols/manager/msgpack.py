...
def with_command(
    func: Callable[
        [BuildbotWebSocketServerProtocol, RemoteCommand | BasicRemoteCommand, dict],
        Coroutine[Any, Any, None],
    ],
) -> Callable[
...
class BuildbotWebSocketServerProtocol(WebSocketServerProtocol):
    ...
    @async_to_deferred
    @with_command
    async def call_update(self, command: RemoteCommand | BasicRemoteCommand, msg: dict) -> None:
        self.ensure_msg_keys(msg, 'args')
        await any_to_async(command.remote_update_msgpack(msg['args']))

    @async_to_deferred
    @with_command
    async def call_complete(self, command: RemoteCommand | BasicRemoteCommand, msg: dict) -> None:
        self.ensure_msg_keys(msg, 'args')
    ...
    async def onConnect(self, request) -> None:
        if self.debug:
            log.msg(f"Client connecting: {request.peer}, {request.headers=}")

        value = request.headers.get('authorization')
        if value is None:
            log.msg("[BuildbotWebSocketServerProtocol] ConnectionDeny!")
