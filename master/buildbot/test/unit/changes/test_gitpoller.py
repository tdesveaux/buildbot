...
class TestGitPollerBareRepository(
    ...
    def setUp(self):
        ...
        if git_bin is None:
            raise unittest.SkipTest("Can't find git binary")
        ...
        self.poller = yield self.attachChangeSource(
            gitpoller.GitPoller(
                self.REPOURL, branches=['main'], workdir=self.poller_workdir, gitbin=git_bin
            )
        )

        self.assert_repository_expecteds_state(testcase=self, git_bin=git_bin)
